...
<body class="body_h100 bg-secondary">
  ...
  if (isset($_POST["jugar"])) {
    //RECOGE LOS NIVELES DEL EQUIPO DEL USUARIO
    $path =getcwd()."/$nick/equipo_usuario.txt";
    $array_digimons = get_digimons($path);
    for ($i=0; $i < count($array_digimons)-1; $i++) {
      $array_niveles[] = $array_digimons[$i][4];
    }
    //FIN RECOGER
    //SELECCIONA 3 DIGIMONS DE LA POKEDEX GLOBAL CON LOS MISMOS
    //NIVELES QUE LOS DIGIMONS DEL EQUIPO USUARIO
    $path =getcwd()."/../Digimons/digimons.txt";
    $array_digimons = get_digimons($path);
    $a=0;
    do {
      $random = rand(0,count($array_digimons)-2);
      $random_nivel = rand(0,count($array_niveles)-1);

      if ($array_digimons[$random][4] == $array_niveles[$random_nivel]) {
        array_splice($array_niveles,$random_nivel,1);//borra el nivel del array de niveles para que no se repita
        $array_equipo_rival[] = $array_digimons[$random];
        $a++;
      }
    } while ($a < 3);
    //FIN DE SELECCION
      ...
      <div class="col-4">
        <div class="vcenter rounded border border-dark p-3">
          ...
          if (isset($_POST["jugar"])) {
            ...
            echo "<table class='table'>
            <tbody>";
            for ($i=0; $i < count($array_equipo_rival); $i++) {//recorre los digimons
              echo "<tr>";
